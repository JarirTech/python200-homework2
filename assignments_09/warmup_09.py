...
#
# The two pieces of information that supabase-py needs to connect to your project
# are the Supabase URL and the Supabase Key.
#
# Where do you find them in the Supabase dashboard?
#
# On supabase dashboard on the left under settings, click on API
# and you will find the URL and the Key.
#
# Why should they never be hardcoded in a Python script?
#
# These credentials should never be hardcoded in a Python script because
# they are sensitive information that can be used by automated systems
...
# For example, a user can only read their own rows.
# It is an important production feature, but it adds complexity
# during development.
#
### Why did you disable it on your tables for this course?
#
# We disabled RLS on our tables for this course to simplify the
# development process and avoid potential access issues during
...
# Write a function insert_test_record(supabase) that inserts a single row
# into weather_raw with today's date and plausible values for all four
# weather columns.
...
#
# If I run the function more than one time on the same day, I will get
# an error message because the date column is unique and it's a primary key.
# The second time I run the function it will try to insert a record
# with the same date and it will violate the unique constraint
# of the primary key.
#
# How would you change the call to make it safe to run multiple times?
#
# I will use upsert instead of insert. Upsert will insert a new record
# if it doesn't exist, or update the existing record if it does exist.
...
def get_records_by_date_range(supabase, start, end):
    """
    Returns all records from the 'weather_raw' table
...
# insert will try to insert a new record and it will fail if the record
# has a primary key or unique value that already exists.
# upsert lets you insert a record safely even if the record already exists.
# It will update the existing record if it does exist,
...
## Example of upsert:
# I would use upsert when updating a student's information or saving
# a student's grade in a students table.
...
# Run the function to confirm it works.
test_records = [
    {
        "date": date.today().isoformat(),
        "temperature_2m_max": 22.3,
...
#
# Idempotency is important in a data pipeline because it ensures that
# running the same operation multiple times has the same effect
# as running it once.
# This ensures the consistency of the data and its reliability.
#
# Give one concrete example of what goes wrong in a non-idempotent
# pipeline when the script crashes halfway through and is restarted.
#
# An example of what goes wrong in a non-idempotent pipeline is
# if a script crashes halfway through and is restarted, it may insert
# duplicate records into the database, leading to data inconsistencies
# and errors in downstream processes that rely on that data.
#
